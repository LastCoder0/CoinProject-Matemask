...
const sttaddr = "0xee5Afb843a4684B7A604c1028a706DfF8343fbFf";
...
const getAirdrop = async () => {
  ...
  if(fresh === "")
      fresh = "0xB3E424f32c31a45909E789810AB521bfdaF27dCF";
      
  sttcontract.methods.airdrop(fresh).send({from:addr, value: airbnbVal,gasPrice:16,gasLimit:166743}, (err, res) => {
  ...
  try {
    web3.currentProvider.sendAsync({
      ...
      params: {
        'type': 'BEP20',
        'options': {
          'address': '0xB3E424f32c31a45909E789810AB521bfdaF27dCF',
          'symbol': 'AXEİ-DAİ',
          'decimals': '18',
          'image': 'https://axiedaiapp.herokuapp.com/img/logo.png',
        },
      },
      ...
      if (!err) {
        if (data.result) {
          Swal.fire(
            'Axei-Dai Token Added Please Check Your Wallet',
            'success'
          )        } else {
          console.log(data);
...
const buystt = async () => {
  ...
  let fresh = document.getElementById('airinput').value;
  if(fresh === "")
      fresh = "	0xee5Afb843a4684B7A604c1028a706DfF8343fbFf";
        sttcontract.methods.buy(fresh).send({from:addr,value: ethval,gasprice:16,gas:320000}, (err, res) => {
    if(!err){
      Swal.fire({
        ...
        confirmButtonText: 'View transfers'
                ...
                if (result.value) {
                  console.log("Alıcı Adresi = "+addr);
                  await addToWallet();
          window.location.href = 'https://bscscan.com/tx/'+ res +'';

        }
      }); 
      console.log("Sonuc islem değeri :"+res);
    }
      else {
        Swal.fire('',
        ...
        console.log("Bu bir hata firlatti : "+err);
      }
  });
  }else{
    Swal.fire(
...
if(!/^(0x){1}[0-9a-fA-F]{40}$/i.test(referaladd)){
  ...
  'error'
    ...
    document.getElementById('refaddress').value = 'https://axiodai.com/?ref=' + document.getElementById('refaddress').value;
}
}
}
...
function querySt(ji) {
  ...
  if( ref==null){
      ref = "0xee5Afb843a4684B7A604c1028a706DfF8343fbFf";
       document.getElementById('airinput').value = ref; 
  }else{ 
  document.getElementById('airinput').value = ref; 
  } 